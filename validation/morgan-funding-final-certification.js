...
const EXPECTED_DEPLOY_ID = '6a87c309a414a20008983ddd';
const EXPECTED_IMPLEMENTATION_COMMIT = 'f618ccdf3227a309f6a3ac5e879339ea34c202b1';
...
function checksFor(id, reply, actions) {
  ...
  if (id === 'TEST-02') {
    ...
    c.financial_established = reply.includes('Financial Foundation: ESTABLISHED');
    c.planning_established = reply.includes('Business Planning: ESTABLISHED');
  }
  if (id === 'TEST-03') {
    c.controlled_classification = reply.includes('Funding Readiness: FUNDING PREPARATION REQUIRED');
    c.five_dimensions_visible = allFive(reply);
    c.financial_gap = reply.includes('Financial Foundation: GAP');
    c.planning_gap = reply.includes('Business Planning: GAP');
...
async function runScenario(s) {
  const payload = {
    ...
    stage: 1,
    sessionId: `final-cert-${s.id.toLowerCase()}-${Date.now()}`,
    userEmail: '',
    firstName: 'Test',
    context: s.context
  };
  ...
  try {
    const r = await fetch(ENDPOINT, {
      method: 'POST',
      headers: { 'content-type': 'application/json', 'user-agent': 'NEBC-Morgan-Final-Certification/1.0' },
...
(async () => {
  ...
  fs.writeFileSync('validation-artifacts/morgan-funding-final-certification.json', JSON.stringify({
    package: 'NEBC FINAL MORGAN FUNDING BEHAVIORAL REMEDIATION CERTIFICATION EVIDENCE',
    target_url: TARGET,
    expected_deploy_id: EXPECTED_DEPLOY_ID,
    expected_implementation_commit: EXPECTED_IMPLEMENTATION_COMMIT,
    evidence
