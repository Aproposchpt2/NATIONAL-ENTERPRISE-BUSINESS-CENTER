...
const BASE = (process.env.MORGAN_PREVIEW_URL || 'https://deploy-preview-6--nat-enterprise-business-center.netlify.app').replace(/\/$/, '');
...
(async () => {
  ...
  if (!passed) {
    console.error('MORGAN_CONNECTIVITY_PRECHECK=FAIL');
    process.exitCode = 1;
  } else {
    console.log('MORGAN_CONNECTIVITY_PRECHECK=PASS');
  }
})();
