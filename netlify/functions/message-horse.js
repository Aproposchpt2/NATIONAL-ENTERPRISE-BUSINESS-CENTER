...
async function resolvePageContext(token, pageId) {
  ...
  try {
    ...
    diag.direct = String(error?.message || error);
  }
  ...
  try {
    ...
    diag.accounts = String(error?.message || error);
  }
...
async function postToFacebook(message) {
  ...
  try {
    ...
    const data = await response.json();
    if (!response.ok) {
      return { posted: false, error: data?.error?.message || `HTTP ${response.status}`, diag: page.diag };
    }
...
async function emailOwner(message, theme, facebook) {
  ...
  if (!key || !to || !from) {
    return { emailed: false, reason: 'RESEND_API_KEY, RESEND_FROM_EMAIL, or MESSAGE_RECIPIENT/RESEND_TO_EMAIL is missing' };
  }
  ...
  const html = `<div style="font-family:Arial,sans-serif;max-width:680px;margin:0 auto;padding:24px;color:#10241c">
    <div style="font-size:12px;letter-spacing:.12em;text-transform:uppercase;color:#9a742d;font-weight:700;margin-bottom:10px">Apropos Message Horse · Daily post</div>
    <h2 style="margin:0 0 8px">${escapeHtml(theme.name)}</h2>
    <div style="font-size:13px;color:#3c5249;margin-bottom:16px">${escapeHtml(status)}</div>
    <div style="background:#fbf9f3;border:1px solid #e3ddcf;border-radius:12px;padding:20px;white-space:pre-wrap;font-size:15px;line-height:1.6">${escapeHtml(message)}</div>
    <div style="margin-top:16px;font-size:13px"><a href="${theme.url}">${theme.url}</a></div>
  </div>`;
  ...
  try {
    const response = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from,
        to: [to],
        subject: `Apropos Message Horse — ${theme.name}`,
        html
      })
    });
    if (!response.ok) {
...
export default async (req) => {
  let dry = false;
  try {
    dry = new URL(req.url).searchParams.get('dry') === '1';
  } catch (_) {}
  ...
  if (!dry && mode === 'paused') {
    return new Response(JSON.stringify({
      ...
      mode,
      skipped: true,
      reason: 'MESSAGE_HORSE_MODE is paused'
  ...
  if (!['preview', 'email', 'post', 'both'].includes(mode)) {
    return new Response(JSON.stringify({
      ran: new Date().toISOString(),
      mode,
      error: 'Unsupported MESSAGE_HORSE_MODE'
  ...
  if (!dry && (mode === 'post' || mode === 'both')) {
    result.facebook = await postToFacebook(message);
  }
  if (!dry && (mode === 'email' || mode === 'both')) {
    result.email = await emailOwner(message, theme, result.facebook);
  }
