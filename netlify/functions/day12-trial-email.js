...
// Runs daily: finds members still on trial whose trial ends approximately two days out.
...
async function sendNudge(m) {
  ...
  const subject = 'Your NEBC free trial ends in 2 days';
  const html = `<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:24px;color:#102033">
    <div style="font-size:12px;letter-spacing:.12em;text-transform:uppercase;color:#b78a31;font-weight:700;margin-bottom:10px">National Enterprise Business Center</div>
    <h2 style="font-family:Georgia,serif;font-size:20px;margin:0 0 12px">${first}, your free trial ends in 2 days.</h2>
    <p style="font-size:15px;line-height:1.6;color:#4d6074">You've been building <b>${biz}</b> inside NEBC. Continue your assessment-led business-development pathway, personalized action planning, Morgan’s Office guidance, and member services for <b>$39 per month</b>.</p>
    <p style="font-size:15px;line-height:1.6;color:#4d6074">Your dashboard, plan, and progress remain available without interruption.</p>
    <a href="${PAY_LINK}" style="display:inline-block;background:#d5aa4d;color:#061b3a;text-decoration:none;font-weight:800;padding:13px 24px;border-radius:10px;margin:8px 0 14px">Continue Membership — $39/month &rarr;</a>
    <p style="font-size:13px;color:#728195">Return to NEBC: <a href="${SITE}" style="color:#0b2b55">${SITE}</a></p>
    <p style="font-size:12px;color:#93a0af;margin-top:18px">National Enterprise Business Center &middot; Cancel anytime before the next billing date.</p>
  </div>`;
  try {
