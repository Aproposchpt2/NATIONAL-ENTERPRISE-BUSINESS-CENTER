...
// Runs daily: finds members still on trial whose trial ends about two days out,
// then sends the verified $39/month continuation notice.
...
async function sendNudge(m) {
  ...
  const subject = 'Your NEBC trial ends in 2 days';
  const html = `<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:24px;color:#10241c">
    ...
    <p style="font-size:15px;line-height:1.6;color:#3c5249">You have been developing <b>${biz}</b> through the National Enterprise Business Center. Continue your Business Readiness Assessment results, personalized action planning, Morgan’s Office guidance, business-development resources, and member progress for <b>$39 per month</b>.</p>
    <p style="font-size:15px;line-height:1.6;color:#3c5249">Your dashboard, report, and progress remain available without interruption.</p>
    <a href="${PAY_LINK}" style="display:inline-block;background:#10623f;color:#fff;text-decoration:none;font-weight:800;padding:13px 24px;border-radius:10px;margin:8px 0 14px">Continue NEBC — $39/month &rarr;</a>
    <p style="font-size:13px;color:#7a8a82">Return to NEBC: <a href="${SITE}" style="color:#10623f">${SITE}</a></p>
    <p style="font-size:12px;color:#9aa8a0;margin-top:18px">National Enterprise Business Center &middot; Monthly billing &middot; Cancel before the next billing date.</p>
  </div>`;
  try {
